...
from net.unet import DoubleConv, Up_DoubleConv, Down_DoubleConv, Up_ThreeResConv, ThreeResConv
...
class Wiener(nn.Module):
    def __init__(self, psf_path, in_chans=1, img_size=(256, 256)):
        ...
        # self.blur_crt_conv = ThreeResConv(in_chans, in_chans, in_chans * 4)
        # self.blur_crt_conv = nn.Conv2d(in_chans, in_chans, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def forward(self, x):
        ...
        # crt_blur_img = self.blur_crt_conv(x)
        # crt_blur_img_fft = img2fft(crt_blur_img)
        # rec_img = fft2img(psf_img_fft_inv * crt_blur_img_fft)
...
class Wiener_psf(nn.Module):
    def __init__(self, psf_path, in_chans=1, img_size=(256, 256)):
        ...
        # self.psf_crt_conv = nn.Sequential(
        #     nn.Conv2d(in_chans, self.mid_chans, kernel_size=3, stride=1, padding=1, bias=False),
        #     nn.BatchNorm2d(self.mid_chans),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(self.mid_chans, self.mid_chans, kernel_size=3, stride=1, padding=1, bias=False),
        #     nn.BatchNorm2d(self.mid_chans),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(self.mid_chans, in_chans, kernel_size=1, stride=1, padding=0, bias=False),
        #     nn.BatchNorm2d(in_chans),
        #     nn.ReLU(inplace=True),
        # )
        # self.psf_crt_conv = ThreeResConv(in_chans, in_chans, in_chans * 6)
    ...
    def forward(self, x):
        ...
        rec_img = resize(rec_img, size=(224, 224))
        rec_img = rec_img.clip(min=0) / rec_img.amax(dim=[1, 2, 3], keepdim=True)
        # rec_img = tF.center_crop(rec_img, 384)
...
class Wiener_psf_rec(nn.Module):
    ...
    def forward(self, x):
        ...
        # crt_rec_img = self.rec_crt_conv(rec_img)
        # crt_rec_img = self.relu(crt_rec_img + rec_img)
        crt_rec_img = crt_rec_img.clip(min=0) / crt_rec_img.amax(dim=[1, 2, 3], keepdim=True)
        # crt_rec_img = crt_rec_img[..., 124: 348, 126:350]
        # crt_rec_img = crt_rec_img[..., 161: 311, 163: 313]
...
class WienerNet(Wiener_ms_psf_rec):
    pass
...
if __name__ == '__main__':
    ...
    # img_blur_s = torch.roll(img_blur, shifts=(4, 2), dims=(-2, -1))
    # img_recons = model.forward(img_blur_s)
    ...
    # plt.imshow(img_recons[75:145, 75:145, ::-1]), plt.title('recons_crop'), plt.axis('off'), plt.show()
    # plt.imshow(img_recons[83:138, 92:137, ::-1]), plt.title('recons_crop'), plt.axis('off'), plt.show()
    # plt.imshow(img_recons[177:295, 197:293, ::-1]), plt.title('recons_crop'), plt.axis('off'), plt.show()
    # plt.imshow(img_recons[72:137, 85:150, ...]), plt.title('recons_crop'), plt.axis('off'), plt.show()
    # plt.imshow(img_recons[184:249, 197:262, ::-1]), plt.title('recons_crop'), plt.axis('off'), plt.show()
    # plt.imshow(img_recons[156:294, 182:320, ::-1]), plt.title('recons_crop'), plt.axis('off'), plt.show()
    # plt.imshow(img_recons[74:136, 81:143, ...]), plt.title('recons_crop'), plt.axis('off'), plt.show()
    # plt.imshow(img_recons[186:248, 193:255, ...]), plt.title('recons_crop'), plt.axis('off'), plt.show()
    # plt.imshow(img_recons[160:290, 174:304, ...]), plt.title('recons_crop'), plt.axis('off'), plt.show()
    plt.close()
    # cv2.imwrite('/home/chenky/datasets/1.png', img_recons[..., ::-1])
